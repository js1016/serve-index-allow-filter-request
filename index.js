      ...
      fs.readdir(path, function (err, files) {
        ...
        if (filter)
          files = files.filter(function (filename, index, list) {
            return filter(filename, index, list, path, req);
          });
        files.sort();
...
serveIndex.html = function _html(
  req,
  res,
  files,
  next,
  ...
  var render =
    typeof template !== "function" ? createHtmlRender(template) : template;
...
serveIndex.json = function _json(
  req,
  res,
  files,
  next,
...
serveIndex.plain = function _plain(
  req,
  res,
  files,
  next,
...
function createHtmlFileList(files, dir, useIcons, view) {
  ...
  html += files
    .map(function (file) {
      ...
      var date =
        file.stat && file.name !== ".."
          ? file.stat.mtime.toLocaleDateString() +
            " " +
  ...
  return function render(locals, callback) {
    ...
    fs.readFile(template, "utf8", function (err, str) {
      ...
      var body = str
        .replace(
          /\{style\}/g,
          locals.style.concat(iconStyle(locals.fileList, locals.displayIcons))
        )
        .replace(
          /\{files\}/g,
          createHtmlFileList(
...
function htmlPath(dir) {
  ...
  for (var i = 0; i < parts.length; i++) {
    ...
    if (part) {
      ...
      crumb[i] =
        '<a href="' +
        escapeHtml(parts.slice(0, i + 1).join("/")) +
        '">' +
        escapeHtml(part) +
        "</a>";
    }
...
function iconStyle(files, useIcons) {
  ...
  for (i = 0; i < files.length; i++) {
    ...
    if (!rules[iconName]) {
      rules[iconName] =
        "background-image: url(data:image/png;base64," + load(iconName) + ");";
      selectors[iconName] = [];
      list.push(iconName);
    }
